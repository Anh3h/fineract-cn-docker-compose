...
function init-config {
    ...
    while IFS="=" read -r key value; do
        case "$key" in
            ...
            "postgres.driverClass") POSTGRES_DRIVER_CLASS="$value" ;;
            "postgres.host") POSTGRES_HOST="$value" ;;
            "postgres.port") POSTGRES_PORT="$value" ;;
            "postgres.user") POSTGRES_USER="$value" ;;
            "postgres.password") POSTGRES_PWD="$value" ;;
            ...
            "payroll-ms.name") PAYROLL_MS_NAME="$value" ;;
            "payroll-ms.description") PAYROLL_MS_DESCRIPTION="$value" ;;
            "payroll-ms.vendor") PAYROLL_MS_VENDOR="$value";;
            "payroll.ip") PAYROLL_IP="$value"; PAYROLL_URL="http://${PAYROLL_IP}:2029/payroll/v1";;
            "group-ms.name") GROUP_MS_NAME="$value" ;;
            "group-ms.description") GROUP_MS_DESCRIPTION="$value" ;;
            "group-ms.vendor") GROUP_MS_VENDOR="$value";;
            "group.ip") GROUP_IP="$value"; GROUP_URL="http://${GROUP_IP}:2029/group/v1";;
            *)
...
create-application $PAYROLL_MS_NAME $PAYROLL_MS_DESCRIPTION $PAYROLL_MS_VENDOR $PAYROLL_URL
create-application $GROUP_MS_NAME $GROUP_MS_DESCRIPTION $GROUP_MS_VENDOR $GROUP_URL
